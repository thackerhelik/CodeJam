/*
We can treat x and y individually
We have to find students at maximum distance in both directions (each for x and y)
take the difference of max and min in each direction and divide by 2 (round up)
*/
...
int main(){
    ...
    while(t--){
    	...
    	sort(x, x + n);
    	sort(y, y + n);
    	cout << max((x[n - 1] - x[0] + 1) / 2, (y[n - 1] - y[0] + 1) / 2) << endl;
    	// ll X = sumx/n;
    	// ll Y = sumy/n;
    	// ll moves = 0;
    	// fo(i, n){
    	// 	moves = max(moves, abs(X - x[i]));
    	// 	moves = max(moves, abs(Y - y[i]));
    	// }
    	// cout << moves << endl;
    }        
